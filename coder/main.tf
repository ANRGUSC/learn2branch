...
resource "coder_agent" "main" {
  ...
  startup_script = <<EOF
    ...
    git clone https://github.com/ANRGUSC/learn2branch.git

    # set vscode theme to dark
    SETTINGS_PATH="/home/vscode/.local/share/code-server/Machine/settings.json"
    mkdir -p $(dirname $SETTINGS_PATH)
    jq '.workbench.colorTheme = "Default Dark+" | .workbench.iconTheme = "vscode-icons"' $SETTINGS_PATH > $SETTINGS_PATH.tmp
    mv $SETTINGS_PATH.tmp $SETTINGS_PATH

    # set default python interpreter to /opt/conda/envs/l2b/bin/python
    jq '.python.pythonPath = "/opt/conda/envs/l2b/bin/python"' $SETTINGS_PATH > $SETTINGS_PATH.tmp
    mv $SETTINGS_PATH.tmp $SETTINGS_PATH
    ...
    # download Github Copilot VSIX
    export publisher=Github
    export extension=copilot
    export version=1.71.8269
    wget -O copilot.vsix https://$publisher.gallery.vsassets.io/_apis/public/gallery/publisher/$publisher/extension/$extension/$version/assetbyname/Microsoft.VisualStudio.Services.VSIXPackage
...
resource "coder_app" "code-server" {
  ...
  name     = "code-server"
  url      = "http://localhost:13337/?folder=/home/vscode/learn2branch"
  icon     = "/icon/code.svg"
}
...
resource "docker_container" "workspace" {
  ...
  image = "jaredraycoleman/learn2branch:latest"
